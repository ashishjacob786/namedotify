...
export default function EditBlog({ params }) {
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) setFormData({ ...formData, featuredImg: data.url });
  ...
  const handleSubmit = async (e, customStatus = null) => {
    ...
    try {
      const res = await fetch('/api/admin/blog', {
        method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(finalData)
      });
    ...
    <div className="max-w-5xl mx-auto font-sans">
      <div className="flex items-center justify-between mb-8">
        <h1 className="text-3xl font-black text-slate-900 flex items-center gap-3">
          <PenTool className="text-blue-600" size={32} /> Edit Article
        </h1>
        <div className="flex gap-3">
          ...
          <button onClick={(e) => handleSubmit(e, 'published')} disabled={loading} className="px-5 py-2.5 rounded-xl font-bold text-white bg-blue-600 hover:bg-blue-700 shadow-md transition flex items-center gap-2 disabled:opacity-70">
            {loading ? <Loader2 className="animate-spin" size={18} /> : <Send size={18} />} Update & Publish
          </button>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
            ...
            <div className="min-h-[400px] text-slate-900">
              <ReactQuill theme="snow" value={formData.content} onChange={(content) => setFormData({...formData, content})} modules={modules} className="h-[350px] rounded-xl text-slate-900" />
            </div>
          </div>
          <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
            <label className="block text-sm font-bold text-slate-700 mb-2">SEO Excerpt</label>
            <textarea rows="3" value={formData.excerpt} onChange={(e) => setFormData({...formData, excerpt: e.target.value})} className="w-full p-4 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none resize-none text-slate-900 font-medium"></textarea>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type="text" value={formData.tags} onChange={(e) => setFormData({...formData, tags: e.target.value})} className="w-full pl-9 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-blue-500 text-sm text-slate-900 font-medium" />
              </div>
            </div>
          </div>
          ...
          <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
            ...
            <div className="mb-4 relative group cursor-pointer">
              ...
              <div className="w-full p-4 border-2 border-dashed border-slate-200 rounded-xl bg-slate-50 group-hover:bg-slate-100 transition flex flex-col items-center justify-center gap-2 text-center">
                {uploading ? <Loader2 className="animate-spin text-blue-500" size={24} /> : <UploadCloud className="text-slate-400 group-hover:text-blue-500" size={24} />}
                <span className="text-sm font-bold text-slate-600">{uploading ? 'Uploading...' : 'Change Image'}</span>
              </div>
            </div>
            ...
            <input type="text" value={formData.imageAlt} onChange={(e) => setFormData({...formData, imageAlt: e.target.value})} className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-blue-500 text-sm font-medium text-slate-900" />
          </div>
