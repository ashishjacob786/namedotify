...
export default function WriteBlog() {
  ...
  const [formData, setFormData] = useState({
    ...
    status: 'published', featuredImg: '', imageAlt: '' // ✅ Added imageAlt
  });
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        setFormData({ ...formData, featuredImg: data.url }); // URL सेव कर लिया
      } else {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
            {/* ✅ FIXED TEXT COLOR */}
            <input type="text" required placeholder="Enter Blog Title Here..." value={formData.title} onChange={handleTitleChange}
              className="w-full text-3xl font-black text-slate-900 placeholder-slate-300 focus:outline-none mb-4 bg-transparent"
            />
          ...
          <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
            ...
            {/* ✅ FIXED TEXT COLOR */}
            <textarea rows="3" placeholder="A short summary for Google Search Results..." value={formData.excerpt} onChange={(e) => setFormData({...formData, excerpt: e.target.value})}
              className="w-full p-4 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none resize-none text-slate-900 font-medium"
            ></textarea>
          </div>
        </div>
